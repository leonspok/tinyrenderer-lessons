...
- (LP4DCoordinate)vertextForFaceElementAtIndex:(NSUInteger)faceElementIx part:(int)part {
	...
	LPVector n = LPVectorApplyTransform(*normal, self.uniformIT);
	
	self->varyingIntensity[part] = MAX(0.0f, LPVectorDotProduct(n, self.lightDirection));
...
- (BOOL)fragment:(LPBaricentricCoordinate)bar color:(TGAColor *)color {
	...
	if (self.model.normalMap) {
		LPVector n = {};
		TGAColor color = [self.model.normalMap getColorAtX:roundf(tc.u*[self.model.normalMap getWidth])
														 y:roundf(tc.v*[self.model.normalMap getHeight])];
		n.dx = color.r-127;
		n.dy = color.g-127;
		n.dz = color.b-127;
		n = LPVectorApplyTransform(n, self.uniformIT);
		intensity = MAX(0.0f, LPVectorDotProduct(n, self.lightDirection));
	}
